...
当前发布基线是 `sendmer v0.10.0`，对应的正式桌面消费端是 `AlterSendmer v0.6.0`；桌面端
通过 crates.io 使用 `sendmer = "0.10.0"`，已完成会话控制、TM1 选择、跨进程回归和原生验收。
上一桌面版本 `v0.5.0` 仍对应 `sendmer = "0.9.0"`。主产品仍是隐私优先的一次性文件传输：
...
核心传输层的 `v0.10.0` 范围已经关闭，AlterSendmer 的 `v0.6.0` 已完成正式适配、回归、
原生验收和发布。下一项工作是独立的桌面质量改进；在没有新的核心契约前，本仓库不为桌面
需求复制协议、缓存或限速实现。
