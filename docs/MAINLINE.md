...
当前发布基线是 `sendmer v0.10.0`，对应的桌面消费端仍是 `AlterSendmer v0.5.0`；桌面端目前
仍依赖 `sendmer = "0.9.0"`，尚未完成 0.10.0 适配。主产品仍是隐私优先的一次性文件传输：
...
| `v0.10.0` | 会话生命周期与资源上限、TM1 manifest、供应链证明和六平台发布覆盖 | 待桌面端升级依赖 |

`AlterSendmer v0.5.0` 使用 `sendmer = "0.9.0"`，只映射公开配置和事件：上传上限以 MiB/s
输入后转换为 bytes/s；持久缓存默认启用并提供 `1 / 7 / 30` 天新条目 TTL 和安全 prune。
缓存格式、锁、恢复状态机和实际限速器仍只由核心传输层维护。桌面端适配必须遵循第 8 节的
跨项目发布顺序，不得用本地 path 或 Git revision 绕过正式的 `sendmer 0.10.0` 版本。
...
核心传输层的 `v0.10.0` 范围已经关闭。下一项跨项目工作是让 AlterSendmer 使用 crates.io
上的 `sendmer 0.10.0` 完成适配、回归与桌面验收；在此之前，本仓库不为桌面需求复制协议、
缓存或限速实现。
